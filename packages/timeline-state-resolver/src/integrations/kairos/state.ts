import { TransitionSelection, TransitionStyle } from 'kairos-connection/dist/enums'
import { SuperSource, TransitionSettings } from 'kairos-connection/dist/state/video'
import { DownstreamKeyer } from 'kairos-connection/dist/state/video/downstreamKeyers'
import { UpstreamKeyer } from 'kairos-connection/dist/state/video/upstreamKeyers'
import { State as DeviceState } from 'kairos-state'
import { assertNever, deepMerge } from '../../lib'
import { KairosStateUtil } from 'kairos-connection'
 ...
 *
 * @param state A DeviceState as is reported by kairos-connection
 * @returns A record of AddressStates
 */
export function kairosStateToAddressStates(state: KairosDeviceState): Record<string, AnyAddressState> {
 ...
 */
export function updateFromKairosState(updateFn: (address: any, state: any) => void, state: DeviceState): void {
	const addressStates = kairosStateToAddressStates(state)
...
export type KairosDeviceState = DeviceState & { controlValues?: Record<string, string> }

/**
 * This function takes in an AddressState and **correctly** (if there are no bugs in the
 * code😉) overrides the DeviceState with it.
 */
export function applyAddressStateToKairosState(deviceState: DeviceState, addrState: AnyAddressState) {
	const getMe = (index: number) => {
		return KairosStateUtil.getMixEffect(deviceState as any, index)
	}
