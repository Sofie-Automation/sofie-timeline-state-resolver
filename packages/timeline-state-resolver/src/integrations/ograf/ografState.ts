...
export function convertTimelineStateToDeviceState(
	...
	for (const [layerId, layer] of Object.entries<Timeline.ResolvedTimelineObjectInstance<TSRTimelineContent>>(
		state.layers
	)) {
		const mapping = mappings[layerId]
		if (!mapping) continue

		if (layer.content.deviceType !== DeviceType.OGRAF) continue
		if (layer.content.type === TimelineContentTypeOgraf.GRAPHIC) {
		...
		} else if (layer.content.type === TimelineContentTypeOgraf.GRAPHIC_STEP) {
			if (mapping.options.mappingType !== MappingOgrafType.RenderTarget) continue
		...
		} else if (layer.content.type === TimelineContentTypeOgraf.RENDERER_CUSTOM_ACTION) {
