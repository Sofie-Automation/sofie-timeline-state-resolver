...
;(async () => {
	...
	for (const p of packages) {
		...
		updateDependencies(p.package.peerDependencies, p.originalName)
...
function translatePackageName(name) {
	if (scopeOverride && name.startsWith('timeline-state-resolver')) {
		let nameSuffix = name
		if (prefixSofie) nameSuffix = 'sofie-' + nameSuffix
